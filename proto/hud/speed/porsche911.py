...
class Porsche911Panel(HudPanel):
    def __init__(self, config=None):
        ...
        self.config.update({
            ...
            'inpaint_radius_ratio': 0.45,  # How far out to inpaint
            'inpaint_angle': 135.0,        # Assumed resting angle of static needle
            'inpaint_wedge_width': 10.0,   # Degrees width to inpaint
        })
    ...
    def _load_and_process_bg(self, size):
        ...
        if self.config.get('inpaint_enabled', True):
            ...
            radius = int(size * self.config.get('inpaint_radius_ratio', 0.45))
            angle = self.config.get('inpaint_angle', 135.0)
            wedge_w = self.config.get('inpaint_wedge_width', 15.0)
            
            # Draw a wedge to cover the needle
            # Convert angles to start/end
            # Ellipse needs start/end angles.
            # Or just draw a thick line? A thick line is easier.
            
            angle_rad = math.radians(angle)
            end_x = int(center[0] + radius * math.cos(angle_rad))
            end_y = int(center[1] + radius * math.sin(angle_rad))
            
            # Needle width varies, usually thicker at base. 
            # Let's draw a triangle or a thick line.
            thickness = int(size * 0.08) # Wide enough to cover
            cv2.line(mask, center, (end_x, end_y), 255, thickness)
            ...
            # Perform inpainting
            # Radius 3 is usually good for small defects
            try:
